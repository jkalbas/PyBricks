...
class Car:
    ...
    def turnToPoint(self,x,y):
        p = self.pose.getPose()
        self.turnAngle(self.angleToPoint(x,y))
    def angleToPoint(self,x,y):
        p = self.pose.getPose()
        return math.atan((p[1]-y)/(p[0]-x))
...
class Pose:
    ...
    def updatePoseDD(self,vR,vL,dt):
        if(vR==vL):
            ...
            self.setPose(xNew,yNew,(self.th+wdt) % (2*math.pi))
class Error:
...
target_pose = Pose(180,180, pi/2)

"""while not car.getTouched():
    car.moveFastWithBias(.9, 0)

car.pullOut()
while car.dFrom(target_pose.getPose()) > 10:
    while not car.getTouched():
        car.moveFastWithBias(speed,car.error.ultraPID(7,0,0,dt),dt)
    if car.getTouched():
        car.pullOut()"""
while car.dFrom(target_pose.getPose()) > 50:
    th = car.pose.getPose()[2]
    tgtTh = car.angleToPoint(180,180)
    eTh = th - tgtTh
    #print(eTh)
    if car.getTouched():
        print("pulling out")
        car.pullOut()
    if(car.ultra.distance()>300 or abs(eTh)<.2):
        print(eTh)
        car.moveFastWithBias(speed,100*eTh,dt)
    else:
        print("following")
        car.moveFastWithBias(speed,car.error.ultraPID(7,0,0,dt),dt)
